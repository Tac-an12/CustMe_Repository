...
const UserHomeForm = () => {
  ...
  const handleEdit = (postId, title, content, images, price, quantity, tags) => {
    navigate(`/posts/${postId}`, { state: { postId, title, content, images, price, quantity, tags } });
  };
  ...
  <div className="flex justify-between items-center mb-2">
    <div className="flex items-center">
      <Avatar alt={post.user.username} sx={{ width: 40, height: 40 }} />
      <div className="ml-3">
    ...
    <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleMenuClose}>
      <MenuItem onClick={() => handleEdit(post.post_id, post.title, post.content, post.images, post.price, post.quantity, post.tags)}>
        Edit
      </MenuItem>
  ...
  <div className="mb-4">
    <Typography variant="body1">
      <SeeMoreText label="Title" text={post.title} />
    </Typography>
    <Typography variant="body1">
      <SeeMoreText label="Content" text={post.content} />
    </Typography>
    <Typography variant="body1">
      Price: {post.price}
    </Typography>
...
const SeeMoreText = ({ text }: {label: string; text: string }) => {
