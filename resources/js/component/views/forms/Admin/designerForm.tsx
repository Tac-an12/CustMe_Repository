...
const DesignerPostForm: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const fetchedDesignerPosts = await fetchDesignerPosts(); // No need to pass page or limit
        setDesignerPosts(fetchedDesignerPosts); // Directly set the fetched posts
      } catch (error) {
        console.error("Error fetching designer posts:", error);
        setDesignerPosts([]); // Set to empty array in case of an error
      }
    };
    fetchPosts();
  }, [fetchDesignerPosts]);
  ...
  const filteredPosts = filterPosts(designerPosts, query);
  ...
  const renderPosts = (posts: any[], title: string) => (
    ...
    <div className="mb-8">
      ...
      <div className="flex justify-center gap-4 flex-wrap ">
        ...
        {/* Render the average rating as stars */}
        <Rating
          name="average-rating"
